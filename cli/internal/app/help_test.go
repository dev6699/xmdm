...
func TestRunPrintsVersionedHelpTree(t *testing.T) {
	...
	out := stdout.String()
	for _, want := range []string{"xmdm 1.2.3", "auth", "login", "whoami", "logout", "enrollment", "tokens", "issue", "validate", "consume", "revoke", "qr", "json", "png", "config", "users", "create", "update", "retire", "files", "managed-files", "certificates", "versions", "publish", "devices", "inspect", "commands", "device-info", "audit", "--base-url"} {
