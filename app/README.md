...
The scaffold already builds, local persistence is in place, bootstrap parsing now persists canonical or fallback payloads, bootstrap state can now flow into enrollment and the initial signed config snapshot, config sync now retries transient failures before caching a verified snapshot and falls back to the secondary server URL when the primary polling path is unavailable, the recovery UI can surface setup failures, and `M3-02 Local Persistence` has passed a physical-device reboot check.
