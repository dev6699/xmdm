...
func resetTestDB(t *testing.T, pool *pgxpool.Pool) {
	...
	_, err := pool.Exec(context.Background(), `
		TRUNCATE TABLE device_info, device_telemetry, enrollment_tokens, managed_files, files, artifacts, app_versions, apps, audit_events, device_groups, devices, policies, groups, users, roles, tenants RESTART IDENTITY CASCADE;
