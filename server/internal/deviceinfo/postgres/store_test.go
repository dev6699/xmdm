...
func TestStoreUploadAndSearchDeviceInfo(t *testing.T) {
	...
	records, err := store.Upload(context.Background(), bootstrap.SeedTenantID, "device-123", "device-secret", deviceinfo.UploadRequest{
		ObservedAt: now,
...
func TestStoreUploadDeviceInfoValidationAndAuth(t *testing.T) {
	...
	store := New(pool)
	if _, err := store.Upload(context.Background(), bootstrap.SeedTenantID, "device-123", "device-secret", deviceinfo.UploadRequest{}); !errors.Is(err, deviceinfo.ErrDeviceInfoInvalid) {
		t.Fatalf("expected invalid device info payload, got %v", err)
	}
	if _, err := store.Upload(context.Background(), bootstrap.SeedTenantID, "missing-device", "device-secret", deviceinfo.UploadRequest{Payload: map[string]any{"model": "Pixel"}}); !errors.Is(err, deviceinfo.ErrDeviceNotFound) {
		t.Fatalf("expected device not found, got %v", err)
	}
	if _, err := store.Upload(context.Background(), bootstrap.SeedTenantID, "device-123", "wrong-secret", deviceinfo.UploadRequest{Payload: map[string]any{"model": "Pixel"}}); !errors.Is(err, deviceinfo.ErrDeviceUnauthorized) {
